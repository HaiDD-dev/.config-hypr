...
Variants {
    ...
    delegate: Component {
        PanelWindow {
            ...
            Process {
                id: updatePoller
                command: ["bash", "-c", "~/.config/hypr/scripts/quickshell/watchers/update_check.sh"]
                stdout: StdioCollector {
                    onStreamFinished: {
                        let txt = this.text.trim();
                        let num = parseInt(txt);
                        if (!isNaN(num)) {
                            barWindow.updateCount = num;
                        }
                    }
                }
            }
            ...
            property string brightPercent: "100%"
            property string brightIcon: "󰛨"
            ...
            property string netDownload: "0KB/s"
            property string netUpload: "0KB/s"
            ...
            // property string kbLayout: "us" // Disabled - keyboard layout not used
            ...
            // --- BRIGHTNESS ---
            Process {
                id: brightPoller; running: true
                command: ["bash", "-c", "~/.config/hypr/scripts/quickshell/watchers/bright_fetch.sh"]
                stdout: StdioCollector {
                    onStreamFinished: {
                        let txt = this.text.trim();
                        if (txt !== "") {
                            try {
                                let data = JSON.parse(txt);
                                let newBright = data.brightness.toString() + "%";
                                if (barWindow.brightPercent !== newBright) barWindow.brightPercent = newBright;
                                if (barWindow.brightIcon !== data.icon) barWindow.brightIcon = data.icon;
                            } catch(e) {}
                        }
                        brightWaiter.running = false;
                        brightWaiter.running = true;
                    }
                }
            }
            Process { id: brightWaiter; command: ["bash", "-c", "~/.config/hypr/scripts/quickshell/watchers/bright_wait.sh"]; onExited: { brightPoller.running = false; brightPoller.running = true; } }
        ...
        Process { id: networkWaiter; command: ["bash", "-c", "~/.config/hypr/scripts/quickshell/watchers/network_wait.sh"]; onExited: { networkPoller.running = false; networkPoller.running = true; } }
            ...
            // --- CPU TEMP ---
            Process {
                id: tempPoller; running: true
                command: ["bash", "-c", "cat /sys/class/thermal/thermal_zone0/temp 2>/dev/null | awk '{printf \"%.0f\", $1/1000}'"]
                stdout: StdioCollector {
                    onStreamFinished: {
                        let txt = this.text.trim();
                        if (txt !== "") {
                            barWindow.cpuTemp = txt + "°";
                        }
                        tempWaiter.running = false;
                        tempWaiter.running = true;
                    }
                }
            }
            Process { id: tempWaiter; command: ["bash", "-c", "~/.config/hypr/scripts/quickshell/watchers/temp_wait.sh"]; onExited: { tempPoller.running = false; tempPoller.running = true; } }

            // --- RAM & DISK ---
            Process {
                id: sysPoller; running: true
                command: ["bash", "-c", "RAM=$(free | grep Mem | awk '{printf \"%.0f\", $3/$2*100}'); DISK=$(df -h / | tail -1 | awk '{print $5}' | tr -d '%'); echo \"$RAM|$DISK\""]
                stdout: StdioCollector {
                    onStreamFinished: {
                        let txt = this.text.trim();
                        if (txt !== "") {
            ...
            Process {
                id: netSpeedPoller
                command: ["bash", "-c", "awk '/^ *(eth|en|wl)/{rx+=$2;tx+=$10} END{print rx\" \"tx}' /proc/net/dev"]
                stdout: StdioCollector {
                    onStreamFinished: {
                        ...
                        if (parts.length >= 2) {
                            ...
                            if (barWindow.netRxPrev > 0) {
                                let rxDelta = Math.max(0, rx - barWindow.netRxPrev);
                                let txDelta = Math.max(0, tx - barWindow.netTxPrev);
                                barWindow.netDownload = barWindow.formatNetSpeed(rxDelta);
                                barWindow.netUpload = barWindow.formatNetSpeed(txDelta);
                            }
                            barWindow.netRxPrev = rx;
                            barWindow.netTxPrev = tx;
                        }
                    }
                }
            }
            ...
            Item {
                ...
                Row {
                    ...
                    // Update Button (Hidden - commented out)
                    // Rectangle {
                    //     property bool isHovered: oldUpdateMouse.containsMouse
                    //     color: isHovered ? Qt.rgba(mocha.surface1.r, mocha.surface1.g, mocha.surface1.b, 0.6) : Qt.rgba(mocha.surface0.r, mocha.surface0.g, mocha.surface0.b, 0.4)
                    //     radius: barWindow.s(10); height: barWindow.s(34);
                    //     clip: true
                    //     
                    //     Rectangle {
                    //         anchors.fill: parent
                    ...
                    //     property real targetWidth: oldUpdateIcon.width + barWindow.s(24)
                    //     width: targetWidth
                    ...
                    //         id: oldUpdateIcon
                    //         anchors.centerIn: parent
                    //         text: "󰚰"; font.family: "Iosevka Nerd Font"; font.pixelSize: barWindow.s(16); 
                    //         color: parent.isHovered ? mocha.text : mocha.overlay2 
                    //     }
                    //     
                    //     MouseArea {
                    //         id: oldUpdateMouse
                    ...
                    //         hoverEnabled: true
                    //         onClicked: {
                    //             barWindow.updateCount = 0;
                    //             Quickshell.execDetached(["bash", "-c", "rm -f ~/.cache/qs_update_pending && alacritty -e sudo pacman -Syu --noconfirm"]);
                    //         }
                    //     }
                    // }
                }            
...
// Left-to-Right train logic (Settings Closed)
                    ...
                    property real settingsX: netSpeedBox.settingsX - targetWidth - barWindow.s(8)
                        ...
                        Repeater {
                            ...
                            delegate: Item {
                                ...
                                Rectangle {
                                    id: wsPill
                                    anchors.verticalCenter: parent.verticalCenter
                                    width: isActive ? barWindow.s(28) : barWindow.s(24)
                                    height: isActive ? barWindow.s(26) : barWindow.s(22)
                                    radius: barWindow.s(6)
                                    color: isActive
                                        ? mocha.blue
                                        : (isOccupied
                                            ? Qt.rgba(mocha.surface0.r, mocha.surface0.g, mocha.surface0.b, 0.85)
                                            : Qt.rgba(mocha.surface1.r, mocha.surface1.g, mocha.surface1.b, 0.25))
                                    border.width: isOccupied && !isActive ? 1 : 0
                                    border.color: Qt.rgba(mocha.overlay1.r, mocha.overlay1.g, mocha.overlay1.b, 0.45)
                                    ...
                                    Text {
                                        ...
                                        font.pixelSize: isActive ? barWindow.s(12) : barWindow.s(11)
                                        font.weight: Font.Bold
                                        color: isActive ? mocha.base : (isOccupied ? mocha.subtext1 : mocha.overlay1)
                                        Behavior on color { ColorAnimation { duration: 200 } }
                                    }
                                }
                                MouseArea {
                                    anchors.fill: parent
                                    onClicked: Quickshell.execDetached(["hyprctl", "dispatch", "workspace", model.wsId])
                                }
                            }
                        }
                    }
                ...
                Rectangle {
                    ...
                    property real targetWidth: netSpeedLayout.implicitWidth + barWindow.s(20)

                    property real defaultX: workspacesBox.targetX + workspacesBox.targetWidth + barWindow.s(8)
                    property real settingsX: tempBox.settingsX - targetWidth - barWindow.s(8)
                    ...
                    Row {
                        ...
                        Column {
                            anchors.verticalCenter: parent.verticalCenter
                            Text { text: "󰁆"; font.family: "Iosevka Nerd Font"; font.pixelSize: barWindow.s(14); color: mocha.green; anchors.horizontalCenter: parent.horizontalCenter }
                            Text { text: barWindow.netDownload; font.family: "JetBrains Mono"; font.pixelSize: barWindow.s(9); font.weight: Font.Bold; color: mocha.text; anchors.horizontalCenter: parent.horizontalCenter }
                        }

                        Column {
                            anchors.verticalCenter: parent.verticalCenter
                            Text { text: "󰁞"; font.family: "Iosevka Nerd Font"; font.pixelSize: barWindow.s(14); color: mocha.peach; anchors.horizontalCenter: parent.horizontalCenter }
                            Text { text: barWindow.netUpload; font.family: "JetBrains Mono"; font.pixelSize: barWindow.s(9); font.weight: Font.Bold; color: mocha.text; anchors.horizontalCenter: parent.horizontalCenter }
                        }
                    }
                }
                ...
                Rectangle {
                    ...
                    Row { 
                        anchors.centerIn: parent; spacing: barWindow.s(8)
                        // Temperature
                        Column {
                            anchors.verticalCenter: parent.verticalCenter
                            Text { 
                                text: "󰔏"; font.family: "Iosevka Nerd Font"; font.pixelSize: barWindow.s(14); 
                                color: mocha.overlay2; anchors.horizontalCenter: parent.horizontalCenter
                            }
                            Text { 
                                text: barWindow.cpuTemp; font.family: "JetBrains Mono"; font.pixelSize: barWindow.s(10); font.weight: Font.Bold; 
                        ...
                        Column {
                            ...
                            Text { 
                                text: barWindow.ramPercent; font.family: "JetBrains Mono"; font.pixelSize: barWindow.s(10); font.weight: Font.Bold; 
                                color: mocha.text; anchors.horizontalCenter: parent.horizontalCenter
                            }
                        }
                        // Disk
                        Column {
                            anchors.verticalCenter: parent.verticalCenter
                            Text { 
                                text: "󰋊"; font.family: "Iosevka Nerd Font"; font.pixelSize: barWindow.s(14); 
                                color: mocha.overlay2; anchors.horizontalCenter: parent.horizontalCenter
                            }
                            Text { 
                ...
                Rectangle {
                    ...
                    property real targetX: netSpeedBox.targetX + netSpeedBox.targetWidth + (targetWidth > 0 ? barWindow.s(8) : 0)
                    x: targetX
                ...
                Rectangle {
                    ...
                    // Left-to-Right train logic (Settings Closed)
                    property real defaultX: tempBox.defaultX + tempBox.targetWidth + barWindow.s(8)
                ...
                Rectangle {
                    ...
                    // Left-to-Right train logic (Settings Closed) - Takes absolute center
                ...
                Row {
                    ...
                    Rectangle {
                        ...
                        Row {
                            ...
                            // KB (Disabled - keyboard layout not used)
                            // Rectangle {
...
radius: barWindow.s(10); height: barWindow.s(34);
                            ...
                            Rectangle {
                                ...
                                onWheel: {
                                    if (wheel.angleDelta.y > 0) {
                                        Quickshell.execDetached(["bash", "-c", "~/.config/hypr/scripts/quickshell/watchers/audio_fetch.sh --up"])
                                    } else {
                                        Quickshell.execDetached(["bash", "-c", "~/.config/hypr/scripts/quickshell/watchers/audio_fetch.sh --down"])
                                    }
                                }
                            }
                            }
                            ...
                            Rectangle {
                                ...
                                Rectangle {
                                    anchors.fill: parent
                                    radius: barWindow.s(10)
                                    opacity: parseInt(barWindow.brightPercent) > 0 ? 1.0 : 0.0
                                    Behavior on opacity { NumberAnimation { duration: 300 } }
                                    gradient: Gradient {
                                        orientation: Gradient.Horizontal
                                        GradientStop { position: 0.0; color: mocha.peach }
                                ...
                                property real targetWidth: brightLayoutRow.width + barWindow.s(24)
                                width: targetWidth
                                Behavior on width { NumberAnimation { duration: 500; easing.type: Easing.OutQuint } }
                                ...
                                Row { 
                                    id: brightLayoutRow; anchors.centerIn: parent; spacing: barWindow.s(8)
                                    Text { 
                                        anchors.verticalCenter: parent.verticalCenter
                                        text: barWindow.brightIcon; font.family: "Iosevka Nerd Font"; font.pixelSize: barWindow.s(16); 
                                        color: parseInt(barWindow.brightPercent) > 0 ? mocha.base : mocha.subtext0 
                                    }
                                    Text { 
                                        anchors.verticalCenter: parent.verticalCenter
                                        text: barWindow.brightPercent; 
                                        font.family: "JetBrains Mono"; font.pixelSize: barWindow.s(13); font.weight: Font.Black; 
                                        color: parseInt(barWindow.brightPercent) > 0 ? mocha.base : mocha.text; 
                                    }
                                }
                                MouseArea { 
                                id: brightMouse; hoverEnabled: true; anchors.fill: parent; 
                                onClicked: Quickshell.execDetached(["bash", "-c", "~/.config/hypr/scripts/qs_manager.sh toggle brightness"])
                                onWheel: {
                                    if (wheel.angleDelta.y > 0) {
                                        Quickshell.execDetached(["bash", "-c", "~/.config/hypr/scripts/quickshell/watchers/bright_fetch.sh --up"])
                                    } else {
                                        Quickshell.execDetached(["bash", "-c", "~/.config/hypr/scripts/quickshell/watchers/bright_fetch.sh --down"])
                                    }
                                }
                            }
                            }
